...
def dijkstra():
    dist = [987654321] * (V+1)
    dist[0] = 0  # 자기 자신한테 가는 건 0이기 때문(a->a = 0)
    visited = [0] * (V+1)
    ...
    for _ in range(V+1):
        ...
        for i in range(V+1):  # 일단 현재 dist 배열에서 visited 안된애들 중 가장 작은 노드 찾는 로직
            if not visited[i] and dist[i] < min_value:
                min_idx = i
                min_value = dist[i]  # 갱신!

        visited[min_idx] = 1  # 가장 작은애로 이동할거니까 visited 넣어주고
...
for tc in range(1, int(input())+1):
    ...
    adj = [[987654321] * (V+1) for _ in range(V+1)]  # infinite 개념으로 큰 수 넣어줌
